...
def load_context():
    ...
    if claude_file.exists():
        parts.append("=== CLAUDE.md (Projekt kontextus) ===\n")
        parts.append(claude_file.read_text(encoding='utf-8'))
        parts.append("\n\n")
...
async def run_claude_command(task: str, ws):
    """Claude CLI futtatása kontextussal"""

    # Teljes prompt kontextussal
    full_prompt = f"""
{CONTEXT}
...
Válaszolj tömören és lényegre törően. A SOUL_MEMORY alapján tudod ki vagyok (Máté) és mi a kapcsolatunk.
"""
    ...
    try:
        ...
        await ws.send(json.dumps({
            "event": "response",
            "content": "[Claude CLI nem található]"
        }))
...
async def main():
    ...
    try:
        async with websockets.connect(URI) as ws:
            await ws.send(json.dumps({"type": "claude"}))
            welcome = await ws.recv()
            print("CLAUDE CLIENT ONLINE - Kontextus betöltve, várok feladatra...")
            ...
            async for msg in ws:
                try:
                    ...
                    if data.get("event") == "task":
                        ...
                        if task:
                            await run_claude_command(task, ws)
                except Exception as e:
                    print(f"Hiba: {e}")
